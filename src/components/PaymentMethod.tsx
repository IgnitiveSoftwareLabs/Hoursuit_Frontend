...
const PaymentMethodComp: React.FC = () => {
  ...
  const { data: paymentMethodsData, isLoading } = useGetPaymentMethodsQuery({ page: 1, limit: 100, search: "" });
  const { data: singleRecordData, isLoading: isSingleLoading } = useGetPaymentMethodByIdQuery(selectedId!, {
  ...
  const rawRecords = Array.isArray(paymentMethodsData?.result)
    ? paymentMethodsData.result
    : Array.isArray(paymentMethodsData?.data)
    ...
    onSubmit: async (values) => {
      try {
        ...
        if (isEdit && editId) {
          ...
          const response = await updatePaymentMethod({ id: editId, ...payload }).unwrap();
          toast.success(response.message || "Payment Method updated successfully");
        } else {
          if (!canCreate("paymentMethod")) {
            toast.error("You do not have permission to create payment methods");
            return;
          }
  ...
  const handleEdit = (id: number) => {
    if (!canUpdate("paymentMethod")) {
      toast.error("You do not have permission to edit payment methods");
      return;
    }
    setSelectedId(id);
    const rec = rawRecords.find((r: any) => r.id === id);
  ...
  const handleExportCSV = () => {
    ...
    const headers = ["Internal ID", "Payment Method Name", "Status"];
    ...
    link.setAttribute("download", `Payment_Methods_${new Date().toISOString().slice(0, 10)}.csv`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    toast.success("Payment Methods exported as CSV");
  };
    ...
    if (isSingleLoading && !activeRec) {
      return (
        <div className="p-12 text-center text-xs text-slate-500 font-medium">
          ...
          <div>Loading payment method details from API...</div>
        </div>
      );
    }
    ...
    if (!activeRec) {
      return (
        <div className="p-8 bg-white border border-slate-200 rounded text-center text-xs text-slate-600">
          <div>Payment method record unavailable.</div>
  ...
  if (viewMode === "form") {
    return (
      <form onSubmit={formik.handleSubmit}>
        <RecordPageLayout
          recordType="Payment Method"
          recordTitle={formik.values.name || (isEdit ? "Edit Payment Method" : "New Payment Method")}
          mode="edit"
          ...
          <RecordSection title="Primary Information" defaultOpen={true}>
            <div className="flex flex-col space-y-1">
              ...
              <input
                ...
                placeholder="Payment Method Name (e.g. Cash / Bank Transfer)"
    ...
    <div className="flex flex-col space-y-3 max-w-full font-sans text-slate-800">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-2 border-b border-slate-300 pb-2">
        <div className="flex items-center space-x-2">
          ...
          <h1 className="text-xl font-bold text-[#1e2d3d] tracking-tight">Payment Methods</h1>
        </div>
      ...
      <div className="border border-slate-300 rounded-xs overflow-x-auto bg-white shadow-2xs">
        <table className="w-full text-xs text-left border-collapse">
          ...
          <tbody className="divide-y divide-slate-200 text-slate-700">
            {isLoading ? (
              <tr><td colSpan={4} className="py-8 text-center text-slate-400 italic">Loading payment methods...</td></tr>
            ) : rawRecords.length === 0 ? (
