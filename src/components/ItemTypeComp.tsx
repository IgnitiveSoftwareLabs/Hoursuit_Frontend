...
const ItemTypeComp: React.FC = () => {
  ...
  const { data: itemTypesData, isLoading } = useGetItemTypesQuery();
  const { data: singleRecordData, isLoading: isSingleLoading } = useGetSingleItemTypeQuery(selectedId!, {
    skip: !selectedId || viewMode !== "view",
  });

  const [createItemType, { isLoading: isCreating }] = useCreateItemTypeMutation();
  const [updateItemType, { isLoading: isUpdating }] = useUpdateItemTypeMutation();
  const [deleteItemType] = useDeleteItemTypeMutation();

  const rawRecords = Array.isArray(itemTypesData?.result)
    ? itemTypesData.result
    : Array.isArray(itemTypesData?.data)
    ? itemTypesData.data
    : Array.isArray(itemTypesData)
    ? itemTypesData
    : [];

  const formik = useFormik<ItemTypeFormValues>({
    initialValues: {
      item_type_name: "",
      description: "",
      isActive: true,
    ...
    validationSchema: Yup.object({
      item_type_name: Yup.string()
        .min(2, "Item Type Name must be at least 2 characters")
        .max(100, "Item Type Name must be at most 100 characters")
    ...
    onSubmit: async (values) => {
      try {
        ...
        if (isEdit && editId) {
          if (!canUpdate("itemType")) {
            toast.error("You do not have permission to update item types");
            return;
          }
          const response = await updateItemType({ id: editId, payload }).unwrap();
          toast.success(response.message || "Item Type updated successfully");
        } else {
          ...
          toast.success(response.message || "Item Type created successfully");
        }
    ...
    if (urlId) {
      ...
      if (urlAction === "edit") {
        ...
        if (rec) {
          ...
          formik.setValues({
            item_type_name: rec.item_type_name || rec.name || "",
  ...
  const handleExportCSV = () => {
    ...
    const headers = ["Internal ID", "Item Type Name", "Description", "Status"];
    const rows = rawRecords.map((r: any) => [
      r.id,
      `"${r.item_type_name || r.name || ""}"`,
      `"${r.description || ""}"`,
    ...
    toast.success("Item Types exported as CSV");
  };

  if (!canRead("itemType")) {
    return (
      <div className="p-6 bg-white rounded border border-slate-200 text-xs text-slate-600">
        You do not have permission to view Item Types.
      </div>
    );
  }
  ...
  if (viewMode === "view") {
    ...
    if (isSingleLoading && !activeRec) {
      return (
        <div className="p-12 text-center text-xs text-slate-500 font-medium">
          ...
          <div>Loading item type details from API...</div>
        </div>
      );
    }
    ...
    if (!activeRec) {
      return (
        <div className="p-8 bg-white border border-slate-200 rounded text-center text-xs text-slate-600">
          <div>Item type record unavailable.</div>
    ...
    return (
      <RecordPageLayout
        ...
        subtitle={`${activeRec.item_type_name || activeRec.name}`}
        mode="view"
      ...
      >
        <RecordSection title="Primary Information" defaultOpen={true}>
          ...
          <div className="flex flex-col space-y-0.5">
            <span className="text-[10px] font-semibold text-slate-500 uppercase">ITEM TYPE NAME</span>
            <span className="text-xs font-bold text-slate-900">{activeRec.item_type_name || activeRec.name}</span>
          </div>
          <div className="flex flex-col space-y-0.5">
  ...
  if (viewMode === "form") {
    return (
      <form onSubmit={formik.handleSubmit}>
        <RecordPageLayout
          recordType="Item Type"
          recordTitle={formik.values.item_type_name || (isEdit ? "Edit Item Type" : "New Item Type")}
          mode="edit"
          ...
          <RecordSection title="Primary Information" defaultOpen={true}>
            <div className="flex flex-col space-y-1">
              <label className="text-[11px] font-semibold text-[#475569] uppercase">
                ITEM TYPE NAME <span className="text-amber-600">*</span>
              </label>
              <input
                type="text"
                name="item_type_name"
                ...
                placeholder="Item Type Name (e.g. Inventory / Service / Non-Inventory)"
    ...
    <div className="flex flex-col space-y-3 max-w-full font-sans text-slate-800">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-2 border-b border-slate-300 pb-2">
        <div className="flex items-center space-x-2">
          ...
          <h1 className="text-xl font-bold text-[#1e2d3d] tracking-tight">Item Types</h1>
        </div>
        ...
        <table className="w-full text-xs text-left border-collapse">
          <thead className="bg-slate-200 text-slate-700 uppercase text-[10px] tracking-wider font-bold border-b border-slate-300 select-none">
            <tr>
              ...
              <th className="px-3 py-2 border-r border-slate-300">ITEM TYPE NAME</th>
          ...
          <tbody className="divide-y divide-slate-200 text-slate-700">
            {isLoading ? (
              <tr><td colSpan={5} className="py-8 text-center text-slate-400 italic">Loading item types...</td></tr>
            ) : rawRecords.length === 0 ? (
              ...
              rawRecords.map((row: any, idx: number) => (
                <tr key={row.id || idx} className={`hover:bg-amber-50/70 ${idx % 2 === 0 ? "bg-white" : "bg-slate-50/50"}`}>
                  ...
                  <td className="px-3 py-1.5 border-r border-slate-200 font-bold text-slate-900">{row.item_type_name || row.name}</td>
